...
def test_all_errors_derive_from_steam_api_error() -> None:
    assert issubclass(SteamConnectionError, SteamApiError)
    assert issubclass(SteamProtocolError, SteamApiError)
    assert issubclass(InvalidTokenError, SteamApiError)
    assert issubclass(SteamLogonError, SteamApiError)
